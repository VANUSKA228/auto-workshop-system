  ...
  return (
    <nav className="bg-primary-dark text-white px-6 py-3 flex items-center justify-between shadow-md">
      <div className="flex items-center gap-1">
        <NavLink to="/orders" className="flex items-center gap-2 mr-4 font-bold text-white text-lg">
          <span>🔧</span> Автосервис
        </NavLink>
        <NavLink to="/orders" className={linkClass}>Заявки</NavLink>
        {(user?.role === 'client' || user?.role === 'admin') && (
          <NavLink to="/services" className={linkClass}>Услуги</NavLink>
        )}
        {(user?.role === 'master' || user?.role === 'admin') && (
          <NavLink to="/workers" className={linkClass}>Работники</NavLink>
        )}
        {user?.role === 'admin' && (
